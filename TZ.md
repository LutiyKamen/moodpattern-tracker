...
1.  **DiaryEntry** (Дневниковая запись)
    ...
    - `mood_score` (FloatField, null=True, blank=True) - числовая оценка настроения от -10 (очень негативное) до 10 (очень позитивное), рассчитанная системой
...
3.  **MoodCorrelation** (Корреляция настроения и темы)
    ...
    - `correlation_score` (FloatField) - коэффициент корреляции между появлением слова в записи и оценкой настроения (от -10 до 10). Отрицательное значение означает, что при упоминании темы настроение ухудшается.
